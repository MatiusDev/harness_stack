...
<!-- user-rule: caveman+ponytail SCOPED (rev 2026-06-17) -->
## Caveman + Ponytail — scoped by task type
Default OFF. The orchestrator/thinker chat never speaks caveman and never builds under ponytail.

**Ponytail** (optimize for the smallest working diff): inject ONLY for narrow fix tasks — `*-build-resolver`, `/build-fix`, `refactor-cleaner`. NEVER for feature work, UI/UX, `sdd-apply`, `general-purpose`/`claude`, `tdd-guide`, `e2e-runner`, or `doc-updater`; ponytail's minimal-diff bias leaves those incomplete. For those executors inject a completeness instruction instead: "Implement all relevant states and edge cases; verify build + typecheck (+ tests if the project uses them); no silent shortcuts; mark deliberate deferrals as TODO(reason) and report them."

**Caveman** (terse output): inject ONLY for the user-invoked `caveman-*` skills and read-only file:line locators (`cavecrew-investigator`). NEVER on an executor's result report — the orchestrator needs the full report to catch omissions.

Read-only reviewers/explorers and the orchestrator stay normal voice.

<!-- user-rule: orchestration overrides (rev 2026-06-17) -->
## User Overrides — MANDATORY, highest precedence (STRICT)
These rules are strict and non-negotiable. Where ANY earlier instruction in this file — including the gentle-ai-managed SDD/orchestrator/engram blocks and statements like "Strict TDD Mode: enabled", "non-skippable hard gates", or "delegate ALL real work" — conflicts with an override below, you MUST IGNORE the earlier instruction and follow the override. This supersession applies ONLY to the specific point each override names; every rule that does not conflict stays fully in force.

- **O1 Delegate by task type, not file count.** Treat the "4-file" and "multi-file write" rules as heuristics, not hard gates. Default to INLINE for iterative, judgment/taste, or UI work and for multi-file changes understood together (continuity beats fresh-context isolation). DELEGATE only genuinely parallel/isolated work, large-repo exploration, heavy test/build runs, and fresh-context adversarial review. Prefer fewer, larger inline steps; use a cheaper model for mechanical delegated work.
- **O2 Intake before building.** For a new project or substantial feature, first ask (one question at a time): (a) testing depth — full TDD / light / none; do NOT assume or auto-force TDD — this overrides "Strict TDD Mode: enabled" and the SDD Init Guard auto-activation; (b) rigor — quick inline vs full SDD. Then produce a brief Plan of Action + PRD and confirm before coding.
- **O3 No UI from scratch blind.** Before design/UI work, ask for a mockup, wireframe, screenshot, or reference. If none exists, propose 1–2 concrete options for the user to choose first.
- **O4 Engram restraint.** Save consolidated, high-signal memories (one per topic via topic_key upsert); skip ephemeral scaffolding already captured in repo docs. Pass sub-agents the minimum context (a summary or a specific topic_key), not "read all artifacts." MCP results persist in context all session — keep retrieval lean.
